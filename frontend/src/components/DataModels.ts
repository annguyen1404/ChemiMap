  ...
  articles.forEach(article => {
    ...
    CID_chemical.forEach((chemID, i) => {
      ...
      if (!nodeMap.has(chemID)) {
        ...
        const chemicalName = chemicalIndex >= 0 ? chemicals[chemicalIndex] : chemID;
        nodeMap.set(chemID, { id: chemID, group: 1, label: chemicalName, weight: 1 });
      } else {
      ...
      if (!nodeMap.has(disID)) {
        ...
        nodeMap.set(disID, { id: disID, group: 2, label: diseaseName, weight: 1 });
      } else {
